...
FullPrecipLoma <- drive_get("FullPrecipLoma 2.csv", shared_drive = "Microbes and Global Change") %>%
...
AnnualSums2 <- FullPrecipLoma %>%
  group_by(WaterYear) %>%
  summarize_at(vars(Ambient,Reduced,Added), sum)

# AnnualSumsCEB <- CEBPrecipLoma %>%
#  group_by(WaterYear) %>%
#  summarize_at(vars(Ambient), sum)
...
plot(x=FullPrecipLoma$Day,y=FullPrecipLoma$Added,xlim=xlim,ylab="Water input (mm)",xlab=NA,type="n")
arrows(FullPrecipLoma$Day,0,FullPrecipLoma$Day,FullPrecipLoma$Added,length=0,angle=90,code=2,lwd=0.5,col="cyan")
...
arrows(FullPrecipLoma$Day,0,FullPrecipLoma$Day,FullPrecipLoma$Ambient,length=0,angle=90,code=2,lwd=0.5,col="red")
arrows(FullPrecipLoma$Day,0,FullPrecipLoma$Day,FullPrecipLoma$Reduced,length=0,angle=90,code=2,lwd=0.5)
...
FullPrecipLomaset <- filter(FullPrecipLoma, Day >= "2020-10-01" & Day <= "2021-09-30")

xlim <- as.Date(c("2020-10-01","2021-09-30"))
pdf("Outputs/LomaPrecipWaterYear2020.pdf",height=6,width=6)
par(cex.axis=1.5,cex.lab=2,font.lab=2,lwd=1.5,tcl=0.4,las=1,mgp=c(3,0.5,0),mar=c(3,5,2,2)+0.1)
plot(x=FullPrecipLomaset$Day,y=FullPrecipLomaset$Added,xlim=xlim,ylab="Water input (mm)",xlab=NA,type="n")
arrows(FullPrecipLoma$Day,0,FullPrecipLoma$Day,FullPrecipLoma$Added,length=0,angle=90,code=2,lwd=0.5,col="cyan")
arrows(FullPrecipLoma$Day,0,FullPrecipLoma$Day,FullPrecipLoma$Ambient,length=0,angle=90,code=2,lwd=0.5,col="red")
arrows(FullPrecipLoma$Day,0,FullPrecipLoma$Day,FullPrecipLoma$Reduced,length=0,angle=90,code=2,lwd=0.5)
legend("topright",c("Ambient","Excluded","Added"),lty=1,col=c("black","red","cyan"),cex=1.5)
...
d <- ggplot(FullPrecipLoma, aes(Day, CumAdded)) + geom_line(color="cyan") + ylab("Water input (mm)") +
